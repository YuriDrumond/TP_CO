...
class Parser:
    ...
    def erro(self, msg):
        raise ErroSintatico(f"[{self.info()}] {msg}")

    def consumir(self, valor_esp=None, tipo_esp=None):
        tok = self.atual()

        if tok[1] == "EOF":
            esperado = valor_esp or tipo_esp
            raise ErroSintatico(
                f"Fim inesperado do arquivo — esperava {esperado}"
            )

        if valor_esp is not None and tok[0] != valor_esp:
            self.erro(
                f"Esperava '{valor_esp}', encontrou '{tok[0]}'"
            )

        if tipo_esp is not None and tok[1] != tipo_esp:
            self.erro(
                f"Esperava {tipo_esp}, encontrou '{tok[0]}'"
            )
    ...
    def parse_tipo(self):
        ...
        if not self.eh_tipo():
            self.erro(
                f"Tipo esperado antes de '{self.valor()}'"
            )
    ...
    def parse_decl_global(self):
        ...
        if not self.eh_tipo():
            self.erro(
                f"Declaração esperada antes de '{self.valor()}'"
            )
        ...
        while self.proximo(offset)[0] == "*":
            offset += 1

        eh_funcao = self.proximo(offset)[0] == "("
        ...
        try:
            ...
            if self.vv("{"):
               ...
               self.consumir(";")
                ...
                self.erro("Else sem if correspodente")
    ...
    def parse_do(self):
        ...
        try:
            ...
            self.erro("Esperava ';' aṕos do-while")
    ...
    def parse_primario(self, contexto="expressão"):
        ...
        if t in {
            ...
            self.erro(
                f"{contexto.capitalize()} inválida perto de '{v}'"
            )
